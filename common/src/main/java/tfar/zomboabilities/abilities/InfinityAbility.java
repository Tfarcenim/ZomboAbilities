...
public class InfinityAbility extends Ability{
    ...
    public void primary(ServerPlayer player) {
        if (player.totalExperience > 1) {
            AbilityUtils.setInfinityActive(player, !AbilityUtils.isInfinityActive(player));
        }
    }
    ...
    public void tickAbility(ServerPlayer player) {
        ...
        if (AbilityUtils.isInfinityActive(player)) {
            if (player.tickCount % TIME == 0) {
                player.giveExperiencePoints(-1);
                if (player.totalExperience ==0) {
                    AbilityUtils.setInfinityActive(player,false);
                }
            }
        }
    }
